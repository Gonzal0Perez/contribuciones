...
' ';')
			...
			if [ -z "$timeerrores" ];then
			    ...
			    aspecout=$(dd if=/dev/random bs=1 count=10 2>/dev/null |$PrPWD/stdtohex|$PrPWD/stddelcar " ")
			    while [ -f "$PaPWD/$aspecout.js" -o -f "$PrPWD/users/tooutput/$userd/$aspecout.js" ];do
				aspecout=$(dd if=/dev/random bs=1 count=10 2>/dev/null |$PrPWD/stdtohex|$PrPWD/stddelcar " ")
			    done
			    echo "" > "$PaPWD/$aspecout.js"
...
' >> "$dirfn/$aspececl.c"
				...
				echo "- * - * - *    $dirfn/$aspececl"
				echo "var ascendente=$ascendente;"  >> "$PaPWD/$aspecout.js"
...
' ' ' >> "$PaPWD/$aspecout.js"
				...
				"$dirfn/$aspececl"  | $PrPWD/stdcarsin "var processed=255;" >> "$PaPWD/$aspecout.js"
				echo "<.<.(.(..  - - - -   ..).).>.>"
				echo "$dirfn/$aspececl.c"
				echo "$PaPWD/$aspecout.js"
			    else
				echo " E R R O R ! ! swe_ecl_v "
			    ...
			    cat $SWESRC/swe_ecl_arcosolar.c | $PrPWD/stdcar "double positions[" > "$dirfn/$aspececl.c"
			    echo "$positions;" >> "$dirfn/$aspececl.c"
			    cat $SWESRC/swe_ecl_arcosolar.c | $PrPWD/stdcdr "double positions[" | $PrPWD/stdcdr ";" >> "$dirfn/$aspececl.c"
			    cat "$dirfn/$aspececl.c" | $PrPWD/stdcar "char planets[" > "$dirfn/$aspececl-1.c"
			    echo "$planets;" >> "$dirfn/$aspececl-1.c"
			    cat "$dirfn/$aspececl.c" | $PrPWD/stdcdr "char planets[" | $PrPWD/stdcdr ";" >> "$dirfn/$aspececl-1.c"
			    cat "$dirfn/$aspececl-1.c" | $PrPWD/stdcar "double aspects" > "$dirfn/$aspececl.c"
			    ...
			    echo "$tjd;" >> "$dirfn/$aspececl-1.c"
			    cat "$dirfn/$aspececl.c" | $PrPWD/stdcdr "double tjd=" | $PrPWD/stdcdr ";" >> "$dirfn/$aspececl-1.c"
			    cat "$dirfn/$aspececl-1.c" | $PrPWD/stdcar "double nacimiento=" > "$dirfn/$aspececl.c"
			    echo "$tjdx;" >> "$dirfn/$aspececl.c"
			    cat "$dirfn/$aspececl-1.c" | $PrPWD/stdcdr "double nacimiento=" | $PrPWD/stdcdr ";" >> "$dirfn/$aspececl.c"
			    mv "$dirfn/$aspececl.c" "$dirfn/$aspececl-1.c"
			    cat "$dirfn/$aspececl-1.c" | $PrPWD/stdcar "double tjd2=" > "$dirfn/$aspececl.c"
			    echo "$tjd + $dias" | bc -l | tr -d '
' >> "$dirfn/$aspececl.c"
			    ...
			    cat $SWESRC/swe_ecl_dpd.c | $PrPWD/stdcar "double positions[" > "$dirfn/$aspececl.c"
			    echo "$positions;" >> "$dirfn/$aspececl.c"
			    cat $SWESRC/swe_ecl_dpd.c | $PrPWD/stdcdr "double positions[" | $PrPWD/stdcdr ";" >> "$dirfn/$aspececl.c"
			    cat "$dirfn/$aspececl.c" | $PrPWD/stdcar "char planets[" > "$dirfn/$aspececl-1.c"
			    echo "$planets;" >> "$dirfn/$aspececl-1.c"
			    cat "$dirfn/$aspececl.c" | $PrPWD/stdcdr "char planets[" | $PrPWD/stdcdr ";" >> "$dirfn/$aspececl-1.c"
			    cat "$dirfn/$aspececl-1.c" | $PrPWD/stdcar "double aspects" > "$dirfn/$aspececl.c"
...
' >> "$dirfn/$aspececl.c"
			    ...
			    cat $SWESRC/swe_ecl_direccionsecundaria.c | $PrPWD/stdcar "double positions[" > "$dirfn/$aspececl.c"
			    echo "$positions;" >> "$dirfn/$aspececl.c"
			    cat $SWESRC/swe_ecl_direccionsecundaria.c | $PrPWD/stdcdr "double positions[" | $PrPWD/stdcdr ";" >> "$dirfn/$aspececl.c"
			    cat "$dirfn/$aspececl.c" | $PrPWD/stdcar "char planets[" > "$dirfn/$aspececl-1.c"
			    echo "$planets;" >> "$dirfn/$aspececl-1.c"
			    cat "$dirfn/$aspececl.c" | $PrPWD/stdcdr "char planets[" | $PrPWD/stdcdr ";" >> "$dirfn/$aspececl-1.c"
			    cat "$dirfn/$aspececl-1.c" | $PrPWD/stdcar "double aspects" > "$dirfn/$aspececl.c"
...
' >> "$dirfn/$aspececl.c"
			    ...
			    if [ -z "$aspectoseclipticoserrores" ];then
				echo "- * - * - *    $dirfn/$aspececl"
				"$dirfn/$aspececl" >> "$PaPWD/$aspecout.js"
				echo "<.<.(.(..  - - - -   ..).).>.>"
				echo "$dirfn/$aspececl.c"
				echo "$PaPWD/$aspecout.js"
			    else
				echo " E R R O R ! ! DIR SECUND "
				echo "$aspectoseclipticoserrores"
				rm  "$fn.lock"
				rm  "$nomprograma.lock-$ps1"
				exit 1
			    ...
			    echo $PaPWD/$aspecout.js $PrPWD/users/tooutput/$userd/$aspecout.js
			    mv -v $PaPWD/$aspecout.js $PrPWD/users/tooutput/$userd/$aspecout.js
			else

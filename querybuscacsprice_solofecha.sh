...
listados="";
listado="";
if [ -d "$PrPWD/msgs/" ];then
    listados=$(echo $PrPWD/msgs|$PrPWD/listadodirectorio_dirs_from_std|$PrPWD/stdbuscaarg_donde '
')
    listado=$(echo $PrPWD/msgs|$PrPWD/listadodirectorio_dirs_from_std)
    salta=0;
    while [ -n "$listados" ];do
	presalta=$(echo "$listados" | $PrPWD/stdcar " ")
	dirn=$(echo "$listado"|$PrPWD/stdcdrn 0$salta | $PrPWD/stdcarn $presalta)
	echo "<$dirn>[$listado]"
...
')
		    variables=";$variables"
		    varos="";
		    varis=$(echo -n "$variables" |$PrPWD/stdbuscaarg_count ";time_t")
		    varos="$varos$varis"
			...
			if [ -n "$list" ];then
			    ...
			    encuentra=$(echo -n "$list" | $PrPWD/stdbuscaarg "fecha=")
			    if [ -n "$encuentra" ];then
				times=$reu
				echo "   *   *   *   *   *   * F $times"
			    fi
				...
				symbol=$(echo "$symbol USDT"|tr -d ' '|tr -d '
')
				echo "  *   *   *   *   *   * S $symbol"
			    fi
			fi
			...
			if [ -n "$symbol" ];then
				...
				echo "0$times - ( $interval * 250 * 60 * 1000)"
				times=$(echo "(0$times*1000) -($interval*250*60 * 1000)"|bc -l)
			    ...
			    cat "EMA.c" | $PrPWD/stdcar "double highs[" > "$archname-EMA.c"
			    highs=$(echo -n "$l3]=$highs };")
			    echo -n "$highs" >> "$archname-EMA.c"
			    cat "EMA.c" | $PrPWD/stdcdr "double highs[" | $PrPWD/stdcdr ";" >> "$archname-EMA.c"
			    cat "$archname-EMA.c" | $PrPWD/stdcar "double lows[" > "$archname-EMA-ii.c"
			    lows=$(echo -n "$l3]=$lows };")
			    echo -n "$lows" >> "$archname-EMA-ii.c"
			    ...
			    echo -n "plot \"$archname.dat\" using 1:3 ls 2 w lines title \"PRECIO\", \"$archnamprice-price0.dat\" using 1:2:(sprintf(\"$status (%s, %f)\", system (sprintf(\"date -d @%d +'%F %T'\", \$1)), \$2)) with labels point ps 2 pt 4 offset char 1,1 notitle, \"$archname-EMA.dat\"  using 1:2  with labels point ps 2 pt 4 offset char 1,1"  >> "$archname.gnuplot"

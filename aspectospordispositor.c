...
#include <stdlib.h>
#include <math.h>
char dispositor[12][10]={{'M','A','R','T','E',0},{'V','E','N','U','S',0},{'M','E','R','C','U','R','I','O',0}, {'L','U','N','A',0}, {'S','O','L', 0}, {'V','E','N','U','S',0},  {'P','L','U','T','O','N'}, {'J','U','P','I','T','E','R',0} , {'S','A','T','U','R','N','O',0}, {'U','R','A','N','O',0} , {'N','E','P','T','U','N','O',0} };
...
char ecliptico_planets[14][2][10]={{{'S','O','L',0}, {'M','A','R','T','E',0}},{{'S','O','L',0}, {'U','R','A','N','O',0}},{{'L','U','N','A',0}, {'U','R','A','N','O',0}},{{'L','U','N','A',0}, {'P','L','U','T','O','N',0}},{{'M','E','R','C','U','R','I','O',0}, {'N','E','P','T','U','N','O',0}},{{'V','E','N','U','S',0}, {'N','E','P','T','U','N','O',0}},{{'V','E','N','U','S',0}, {'P','L','U','T','O','N',0}},{{'M','A','R','T','E',0}, {'U','R','A','N','O',0}},{{'J','U','P','I','T','E','R',0}, {'V','E','N','U','S',0}},{{'S','A','T','U','R','N','O',0}, {'L','U','N','A',0}},{{'S','A','T','U','R','N','O',0}, {'V','E','N','U','S',0}},{{'U','R','A','N','O',0}, {'M','A','R','T','E',0}},{{'N','E','P','T','U','N','O',0}, {'V','E','N','U','S',0}},{{'N','E','P','T','U','N','O',0}, {'J','U','P','I','T','E','R',0}}};
  ...
  double positions_ecliptico[10]={121.54986241, 173.74914574, 133.80428291, 111.71850198, 178.81234154, 19.09505726, 321.14080712, 153.52621070, 222.88895044, 160.53177399};
  char planets_names[10][10]={{'S','O','L',0},{'L','U','N','A',0},{'M','E','R','C','U','R','I','O',0},{'V','E','N','U','S',0},{'M','A','R','T','E',0},{'J','U','P','I','T','E','R',0}, {'S','A','T','U','R','N','O',0}, {'U','R','A','N','O',0}, {'N','E','P','T','U','N','O'}, {'P','L','U','T','O',0}};
...
int main(int argc, char *argv[]) {
  ...
  while(j<2) {
    i=0;
    while (i<(sizeof(ecliptico_planets)/sizeof(ecliptico_planets[0][0]))/2) {
      k=0;
      while(k<sizeof(planets_names)/sizeof(planets_names[0])) {
	n=0;
	while(n<sizeof(planets_names[0])) {
	  if(planets_names[k][n]!=ecliptico_planets[i][j][n]) {
	    k=100;
	    break;
	  }
	  if(planets_names[k][n]==0 && n>0) {
	    o[i][j]=(int)fmodl(positions_ecliptico[i], 30);
	    k=100;
	    break;
	  }
	  n++;
  ...
  while (i<sizeof(ecliptico_planets)/sizeof(ecliptico_planets[0])) {  
    k=0;
    //                                B                  disp de      A     disp
    while(k<sizeof(ecliptico_planets[i][1]) && k<sizeof(dispositor[o[i][0]]) && k<3) {
      //                      B           es el disp de A
      if(ecliptico_planets[i][1][k]!=dispositor[ o[i][0] ][k]) break;
      k++;
    }
    if(k==3) {
	...
	k=0;
	while(k<sizeof(ascencional_planets[j][0]) && k<sizeof(dispositor[o[i][0]]) && k<3) {
	  if(ascencional_planets[j][0][k]!=dispositor[o[i][0]][k]) break;
	  k++;
	}
	if(k==3) {
	  n=0;
	  while(n<sizeof(dispositor)/sizeof(dispositor[0])) {
	    ...
	    while(k<sizeof(dispositor[0][1]) && k<3) {
	      if(dispositor[n][k]!=ascencional_planets[j][1][k]) break;
	      k++;
	    }
	    if(k==3) {
	      ecliptico_origen[l]=i;
	...
	if(j<254) {
	  k=0;
	  while(k<sizeof(ascencional_planets[j][1]) && k<sizeof(dispositor[o[i][0]]) && k<3) {
	    if(ascencional_planets[j][1][k]!=dispositor[o[i][0]][k]) break;
	    k++;
	  }
	  if(k==3) {
	    n=0;
	    while(n<sizeof(dispositor)/sizeof(dispositor[0])) {
	      k=0;
	      while(k<sizeof(dispositor[0][1]) && k<3) {
		if(dispositor[n][k]!=ascencional_planets[j][0][k]) break;
		k++;
	      }
	      if(k==3) {
		ecliptico_origen[l]=i;
  ...
  while (i<sizeof(ecliptico_planets)/sizeof(ecliptico_planets[0])) {  
    k=0;
    //                                A                  disp de      B     disp
    while(k<sizeof(ecliptico_planets[i][0]) && k<sizeof(dispositor[o[i][1]]) && k<3) {
      //                      A           es el disp de B
      if(ecliptico_planets[i][0][k]!=dispositor[ o[i][1] ][k]) break;
      k++;
    }
    if(k==3) {
      dispositores_aspectos[l]=0;
      ...
      while(j<sizeof(ascencional_planets)/sizeof(ascencional_planets[0])) {
	k=0;
	while(k<sizeof(ascencional_planets[j][0]) && k<sizeof(dispositor[o[i][1]]) && k<3) {
	  if(ascencional_planets[j][0][k]!=dispositor[o[i][1]][k]) break;
	  k++;
	}
	if(k==3) {
	  n=0;
	...
	if(j<254) {
	  k=0;
	  while(k<sizeof(ascencional_planets[j][1]) && k<sizeof(dispositor[o[i][1]]) && k<3) {
	    if(ascencional_planets[j][1][k]!=dispositor[o[i][1]][k]) break;
	    k++;
	  }
	  if(k==3) {
	    n=0;
  ...
  k=0;
  while(k<l) {
    printf("{\"%s\", \"%s\"}", dispositor[dispositores_planets[k][0]], dispositor[dispositores_planets[k][1]]);
    k++;
    if(k<l) printf(",");
  }
  printf("};\n};");

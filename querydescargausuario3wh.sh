...
echo ""  | tr -d '
' > $nomprograma.lis
curl -L "$remotepath/dirlist.php" > $nomprograma.list 2>/dev/null
while [ -n "$encuentra" ];do
    filename=$(cat "$nomprograma.list" | $PrPWD/stdcdr "<a href=\""|$PrPWD/stdcarsin "\"")
    encuentra=$(echo "$filename"|$PrPWD/stdbuscaarg ".js")
    if [ -n "$encuentra" ];then
#	echo "$filename"
	echo "$filename" >> $nomprograma.lis
    fi
    cat "$nomprograma.list" | $PrPWD/stdcdr "<a href=\""|$PrPWD/stdcdr "\"" > $nomprograma.list.tmp
    cp "$nomprograma.list.tmp" "$nomprograma.list"
    encuentra=$(cat "$nomprograma.list" | $PrPWD/stdbuscaarg "<a href=\"")
done
...
if [ -n "$ttest" ];then
    ...
    curl -L "$remotepath/$fn" > "$fn"  2>/dev/null
    len=$(cat "$fn"|wc -c|tr -d ' '|tr -d '
')
    if [ 0$len -eq 14 ];then
	texto=$(cat "$fn")
	if [ "$texto" = "processed=255;" ];then
	    echo ";$fn;" >> $nomprograma.memoria
	    exit 1;
	fi
    fi
    if [ -f "$fn" ];then
	...
	if [ 0$opens -eq 0  ];then
	    echo " . . . . . . . . . . . $len "
	    if [ 0$len -gt 100 ];then
		echo ";$fn;" >> $nomprograma.memoria
	    fi
	fi
...
' ';')
			...
			if [ -n "$userfromfile" -a -n "$ivfromfile" -a -n "$encryptedfromfile" -a -n "$filedfromfile" -a -z "$ca5" ];then
				...
				dondes=0
				iv=$(echo "$variables" | $PrPWD/stdcdr "$ivfromfile["  | $PrPWD/stdcarsin ";");
				msj=$(echo "$variables" | $PrPWD/stdcdr "$encryptedfromfile["|$PrPWD/stdcarsin ";");
...
' > $datosdelc.c
				    ...
				    echo ">>>> '!!!!!'          $errores <<<<"
				    echo ">> $PaPWD/$outputdelc-bin || $PrPWD/users/$usuarioo/$usuarioo-aes.c"
				    if [ -z "$errores" ];then
					mkdir $PrPWD/users
...
' > $datosdelc.js
			    ...
			    cp $datosdelc.c $PrPWD/users/input/$usuarioo/
			    cp "$fn.c" "$PrPWD/users/input/unencrypted"
			    echo "$errores"
			fi
		    fi
		fi

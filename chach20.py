...
def run_tests():
  ...
  grados=(int(bytearraytohexstring(f), 16) % 360000)
  da=datetime.datetime.strptime(date[0:date.index(".")] + " UTC", "%Y-%m-%dT%H:%M:%S %Z")
  l=round(time.mktime(da.timetuple()))
  ignore=json.loads(ignores.ignore)
  i=0
  m=l
  events=[]
  while(m<l+(3600*24)):
    m+=120
    while(i<len(ignore)):
      if(m > ignore[i]["start"] and m < ignore[i]["end"]): break
      i=i+1
    if(i==len(ignore)): events.append(m)
  text=("var events=[" + ",".join(str(x) for x in events)  + "];\n var date=\"" + date + "\";\n" + "var longNumber = \"0x" + number + "\";\n" + "var key = \"0x" + c + "\";\n" + "var data = \"0x" + d + "\";\n" + "var grados=" + str(grados) + ";\n")

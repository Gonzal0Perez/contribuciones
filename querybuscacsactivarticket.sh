...
' ';')
			...
			echo "$mensaje"
			curl -L -v "$remotepath/$url.png" -o "$PaPWD/$url.png"
			if [ -f "$PaPWD/$url.png" ];then
			    aspecas=$(dd if=/dev/random bs=1 count=10 2>/dev/null |$PrPWD/stdtohex|$PrPWD/stddelcar " ")
			    while [ -f "$PaPWD/$aspecas.c" ];do
				aspecas=$(dd if=/dev/random bs=1 count=10 2>/dev/null |$PrPWD/stdtohex|$PrPWD/stddelcar " ")
			    done
			    touch $PaPWD/$aspecas
			    encuentra=$(echo "$mensaje"| $PrPWD/stdbuscaarg "usar")
			    if [ -n "$encuentra" ];then
			    ...
			    encuentra=$(echo "$mensaje"| $PrPWD/stdbuscaarg "activar")
			    if [ -n "$encuentra" ];then
